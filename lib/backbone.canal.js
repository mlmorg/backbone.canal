(function () {
  ...
  _.extend(Backbone.Router.prototype, {
    ...
    route: function (route, name, callback) {
      ...
      // Get filter methods to be run for this route
      if (name) {
        var that = this;
        var before = _.tap([], function (before) {
          _.each(that.before, function (options, filterName) {
            var add = true;
            options = options || {};
            ...
            if (add && that[filterName]) {
              before.push(that[filterName]);
            }
          });
        });
      }
